        ...
        while (loopCount < maxLoops) {
            ...
            if (log_level >= LogLevel::VERBOSE) {
                std::cout << "  Outer Loop " << loopCount << ": phase=" << (int)state.phase
                          << " (" << (state.phase == Phase::VOTE_ROUND1 ? "VOTE_R1" : state.phase == Phase::VOTE_ROUND2 ? "VOTE_R2"
                                                                                  : state.phase == Phase::DISCARDING    ? "DISCARD"
                                                                                  : state.phase == Phase::PLAYING       ? "PLAYING"
                                                                                  : state.phase == Phase::ENDED         ? "ENDED"
                                                                                                                        : "LOBBY")
                          << "), turn=" << state.turn
...
TEST_F(ScenarioTest, ScoresUpdateAfterRound) {
    ...
    // Get final state
    auto &final_state = runner.getState();

    // The scores should be valid (each team's score is between 0 and 20)
    // In a single hand (or series if multiple hands played), valid scores are 0-20
    EXPECT_GE(final_state.scores[0], 0);
    EXPECT_GE(final_state.scores[1], 0);
    EXPECT_LE(final_state.scores[0], 20);
    EXPECT_LE(final_state.scores[1], 20);
}
